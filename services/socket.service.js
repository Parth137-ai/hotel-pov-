/**
 * localStorage-based State Service
 * Replaces Socket.io for static/Vercel deployment.
 * For cross-device real-time sync, deploy on Railway with server.js.
 */

const STORAGE_KEY = 'gp_pos_state';
...
// Initialize table statuses in default state
...
function getState() {
  try {
    ...
    if (saved) {
      ...
      // Always use default STAFF (don't let localStorage override staff list)
...
// Dummy socket object for compatibility
export const socket = { on: () => {}, emit: () => {} };

export function initSocketListeners(callbacks) {
  // Load state from localStorage and immediately call onInitialState
  const state = getState();
  // Small timeout to ensure DOM is ready
  setTimeout(() => callbacks.onInitialState(state), 50);
}
...
export function notifyOrderReady(msg) {
  // Local notification only (no cross-device in static mode)
