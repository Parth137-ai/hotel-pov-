...
POS_STATE.TABLES.forEach(t => {
  POS_STATE.tableStatus[t.id] = {status:'free',amount:0,items:0,since:null,order:{}};
});
...
function loadData() {
  if (fs.existsSync(DATA_FILE)) {
    try {
      ...
      console.error('Error parsing pos_data.json:', e.message);
    }
  } else {
    saveData();
  }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ Grand Palace POS Server running at http://localhost:${PORT}`);
});
